...
class RepositorySourceFile extends RepositoryLicensedFile {
  ...
  static final RegExp _buildTimePattern = new RegExp(r'^(?!.*gen$)(?:CMakeLists\.txt|Makefile(?:\.am|\.in|)|configure(?:\.ac|\.in)?|config\.(?:sub|guess)|.+\.m4|install-sh|.+\.sh|.+\.bat|.+\.py|icu-configure|.+\.gypi?|.+\.gni?|.+\.gradle|.+\.yaml)$', caseSensitive: false);
  static final RegExp _docsPattern = new RegExp(r'^(?:INSTALL|NEWS|ChangeLog|.+\.txt|.+\.md|.+\.log|.+\.css|.+\.1|doxygen\.config|.+\.spec(?:\.in)?)$', caseSensitive: false);
  static final RegExp _devPattern = new RegExp(r'^(?:codereview\.settings)$', caseSensitive: false);
  ...
  Iterable<License> get licenses {
    ...
    _licenses = determineLicensesFor(contents, name, parent);
    ...
    _licenses.forEach((License license) => license.markUsed(io.fullName));
...
abstract class RepositorySingleLicenseFile extends RepositoryLicenseFile {
  ...
  @override
  bool get isUsed => license.isUsed;
...
class RepositoryGeneralSingleLicenseFile extends RepositorySingleLicenseFile {
  RepositoryGeneralSingleLicenseFile(RepositoryDirectory parent, fs.TextFile io)
    : super(parent, io, new License.fromBodyAndName(io.readString(), io.name));
...
class RepositoryApache4DNoticeFile extends RepositorySingleLicenseFile {
  ...
  static License _parseLicense(fs.TextFile io) {
    ...
    return new License.fromMultipleBlocks(<String>[match.group(1), match.group(2)], LicenseType.apacheNotice);
  }
}
...
class RepositoryLicenseFileWithLeader extends RepositorySingleLicenseFile {
  ...
  static License _parseLicense(fs.TextFile io, RegExp leader) {
    ...
    return new License.fromBodyAndName(body.substring(match.end), io.name);
  }
}
...
class RepositoryReadmeIjgFile extends RepositorySingleLicenseFile {
  ...
  static License _parseLicense(fs.TextFile io) {
    ...
    return new License.message(body, LicenseType.ijg);
  }
...
class RepositoryLibJpegTurboLicense extends RepositoryLicenseFile {
  ...
  static void _parseLicense(fs.TextFile io) {
    String body = io.readString();
    if (!body.contains(_pattern))
      throw 'unexpected contents in libjpeg_turbo LICENSE';
  }
  ...
  @override
  bool get isUsed => licenses.every((License license) => license.isUsed);
...
class RepositoryDartLicenseFile extends RepositorySingleLicenseFile {
  ...
  static License _parseLicense(fs.TextFile io) {
    ...
    return new License.template(match.group(1), LicenseType.bsd);
  }
...
class RepositoryFreetypeLicenseFile extends RepositoryLicenseFile {
  ...
  @override
  bool get isUsed => licensesFor(name).single.isUsed;
...
class RepositorySkiaLicenseFile extends RepositoryLicenseFile {
  RepositorySkiaLicenseFile(RepositoryDirectory parent, fs.TextFile io)
  ...
  static List<License> _parseLicense(fs.TextFile io) {
    // Files of this type should have multiple licenses
    // split with lines of 80 "-" characters
    // with each block except the first starting with
    // a summary of what the license applies to.
    final List<String> contents = io.readString().split('\n${"-" * 80}\n');
    if (contents.length != 3 ||
        (!contents[1].startsWith('third_party/etc1 is under the following license:\n')) ||
        (!contents[2].startsWith('Some files under resources are under the following license:\n')))
      throw 'unexpected text in Skia license file';
    // We skip the third_party/etc1 block because the files in third_party/etc1
    // define their own licenses so this would just be a redundant copy of the
    // Apache license.
    return <License>[
      new License.fromBodyAndType(contents[0], LicenseType.bsd),
      new License.unique(contents[2], LicenseType.unknown),
    ];
  }
  ...
  @override
  License licenseWithName(String name) {
    throw 'tried to use skia multi-license license file to find a license by name';
  }

  @override
  bool get isUsed => _licenses.every((License license) => license.isUsed);
...
class RepositoryMultiLicenseNoticesForFilesFile extends RepositoryLicenseFile {
  ...
  static Map<String, License> _parseLicense(fs.File io) {
    ...
    while (index < contents.length) {
      ...
      License license = new License.unique(bodyText, LicenseType.unknown);
  ...
  @override
  bool get isUsed => _licenses.values.every((License license) => license.isUsed);
...
class RepositoryIcuLicenseFile extends RepositoryLicenseFile {
  ...
  static List<License> _parseLicense(fs.TextFile io) {
    ...
    final List<License> result = <License>[
      new License.fromBodyAndType(_dewrap('${match.group(1)}\n\n${match.group(2)}'), LicenseType.mit),
      new License.fromBodyAndType(_dewrap(match.group(3)), LicenseType.unknown),
      new License.fromBodyAndType(_dewrap(match.group(4)), LicenseType.bsd),
      new License.fromBodyAndType(_dewrap(match.group(5)), LicenseType.bsd),
      new License.fromBodyAndType(_dewrap(match.group(6)), LicenseType.bsd),
      new License.fromBodyAndType(_dewrap(match.group(7)), LicenseType.unknown),
      new License.fromBodyAndType(_dewrap(match.group(8)), LicenseType.unknown),
      new License.fromBodyAndType(_dewrap(match.group(9)), LicenseType.bsd),
      new License.fromBodyAndType(_dewrap(match.group(10)), LicenseType.bsd),
    ];
    return result;
  }
  ...
  @override
  License licenseOfType(LicenseType type) {
    throw 'tried to use ICU license file to find a license by type';
  }
...
class RepositoryDirectory extends RepositoryEntry implements LicenseSource {
  ...
  static final RegExp _licenseNamePattern = new RegExp(r'^(?!.*\.py$)(?!.*(?:no|update)-copyright)(?!.*mh-bsd-gcc).*\b_*(?:license|copying|copyright|notice|l?gpl|bsd|mpl?|ftl\.txt)_*\b', caseSensitive: false);
      ...
      if (shouldRecurse(entry)) {
        ...
        if (entry is fs.Directory) {
          ...
          try {
            ...
            print('failed to handle $entry: $e');
            rethrow;
          }
        } else {
  ...
  License _nearestAncestorLicenseWithName(String name, { String authors }) {
    ...
    if (result != null)
      return result;
    if (parent != null && (authors != null || isLicenseRootException || (!isLicenseRoot && !parent.subdirectoriesAreLicenseRoots)))
    ...
    for (RepositoryLicensedFile file in _files) {
      if (file.isIncludedInBuildProducts) {
        try {
          ...
          print('error searching for copyright in: ${file.io}\n$e');
          if (e is! String)
            print(stack);
          print('\n');
...
class RepositoryAndroidSdkPlatforms22Directory extends RepositoryDirectory {
  RepositoryAndroidSdkPlatforms22Directory(RepositoryDirectory parent, fs.Directory io) : super(parent, io);
...
class RepositoryAndroidSdkPlatformsDirectory extends RepositoryDirectory {
  ...
  @override
  RepositoryDirectory createSubdirectory(fs.Directory entry) {
    if (entry.name == 'android-22')
      return new RepositoryAndroidSdkPlatforms22Directory(this, entry);
    throw 'unknown Android SDK version';
  }
}
...
class RepositoryFreetypeBuildsDirectory extends RepositoryDirectory {
  RepositoryFreetypeBuildsDirectory(RepositoryDirectory parent, fs.Directory io) : super(parent, io);

  @override
  bool shouldRecurse(fs.IoNode entry) {
    // When we add support for Amiga, Atari, or Windows, we should
    // revisit the status of these directories.
    return entry.name != 'amiga'
        && entry.name != 'atari'
        && entry.name != 'windows'
        && super.shouldRecurse(entry);
  }
...
class RepositoryFreetypeSrcDirectory extends RepositoryDirectory {
  ...
  RepositoryDirectory createSubdirectory(fs.Directory entry) {
    if (entry.name == 'builds')
      return new RepositoryFreetypeBuildsDirectory(this, entry);
    if (entry.name == 'docs')
...
class RepositorySkiaDirectory extends RepositoryDirectory {
  ...
  RepositoryFile createFile(fs.IoNode entry) {
    if (entry.name == 'LICENSE')
      return new RepositorySkiaLicenseFile(this, entry);
    return super.createFile(entry);
  }
...
class RepositoryBaseThirdPartyDirectory extends RepositoryGenericThirdPartyDirectory {
  ...
  @override
  bool shouldRecurse(fs.IoNode entry) {
    return entry.name != 'valgrind' // unopt engine builds only
        && super.shouldRecurse(entry);
  }
}
...
class RepositoryRoot extends RepositoryDirectory {
  ...
  RepositoryDirectory createSubdirectory(fs.Directory entry) {
    ...
    if (entry.name == 'mojo')
      return new RepositoryExcludeSubpathDirectory(this, entry, const <String>['public', 'tools', 'bindings', 'generators']);
    if (entry.name == 'sky')
...
class Progress {
  ...
  @override
  String toString() {
    return '${(_withLicense + _withoutLicense).toString().padLeft(10)} of $max ($_withoutLicense missing licenses)  $label    ';
  }
}
...
void main(List<String> arguments) {
  ...
  try {
    ...
    licenses.sort((License a, License b) => a.body.compareTo(b.body));
    progress.label = 'Done.';
    bool done = false;
    assert(() {
      print(licenses.join('\n\n'));
    ...
    if (!done)
      print(licenses.map((License license) => license.body).join('\n${"-" * 80}\n'));
    assert(() {
...
// convert /usr/local/google/home/ianh/dev/engine/src/third_party/libjpeg_turbo/LICENSE.md according to advice
// README.ijg includes lots of non-license text that would be nice to omit (like "file format wars" stuff)
// if a license block contains a reference to the apache license URL we've probably failed
// Ditto for the following lines:
...
// ~/dev/engine/src/sky/engine/core/rendering/style/ShapeValue.h has wrong license right now


// ~/dev/engine/src/third_party/skia/third_party/freetype/include/freetype-android/ftoption.h
// -> failed to find accompanying "LICENSE.TXT"

// /usr/local/google/home/ianh/dev/engine/src/third_party/skia/src/opts/SkBitmapProcState_matrixProcs_neon.cpp
